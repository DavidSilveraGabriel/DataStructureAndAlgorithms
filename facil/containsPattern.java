/*Given an array of positive integers arr,  find a pattern of length m that is repeated k or 
more times.

A pattern is a subarray (consecutive sub-sequence) that consists of one or more values, 
repeated multiple times consecutively without overlapping. A pattern is defined by its length 
...
class Solution {
    public boolean containsPattern(int[] arr, int m, int k) {
        int len = arr.length;
        int i =0,j=i+m,repeat = 0;
        while(j<len){
            if(arr[i]!=arr[j]){
                repeat=0;
            }else if(++repeat==(k-1)*m){
                return true;
            }
            i++;
            j++;
