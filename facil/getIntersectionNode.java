...
the two lists intersect). From the head of A, it reads as [4,1,8,4,5]. From the head of B,
it reads as [5,6,1,8,4,5]. There are 2 nodes before the intersected node in A; There are 3
...
[3,2,4]. There are 3 nodes before the intersected node in A; There are 1 node before the intersected 
node in B.
...
public class Solution {
    ...
    //Agregar cada referencia del nodo de la lista A en el hashset
    while(headA!=null){
        H.add(headA);
        headA=headA.next;
    }
    while(headB!=headA || headB!=null){
        //si hashset contiene la misma referencia, entonces se produjo la intersección
        if(H.contains(headB))
            break;
        headB=headB.next;
    }
